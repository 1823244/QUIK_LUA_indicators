...
local Fsettings=(Fsettings or {})
	...
	if CandleExist(I,ds) then
		...
		if it.l > 1 then
			local Price = GetValue(it.p,CLOSE,ds)
			local PrevPrice = GetValue(it.pp,CLOSE,ds)
			local TH = math.max(PrevPrice, GetValue(it.p,HIGH,ds))
			local TL = math.min(PrevPrice, GetValue(it.p,LOW,ds))
			local WADn = 0
