...
local MaxStep = (Fsettings.MaxStep or 0.2)
	...
	if CandleExist(I,ds) then
		...
		cand.p = {H = GetValue(it.p,HIGH,ds), L = GetValue(it.p,LOW,ds)}
		if it.l==2 then
			tmp.p = {Val = GetValue(it.p,HIGH,ds), Step = Step, Ext = cand.p.H, Long = true}
		end
		if it.l > 2 then
			local Revers = false
			...
			if not Revers then
				cand.pp = {H = GetValue(it.pp,HIGH,ds), L = GetValue(it.pp,LOW,ds)}
				cand.ppp = {H = GetValue(it.ppp,HIGH,ds), L = GetValue(it.ppp,LOW,ds)}
				if tmp.pp.Long then
