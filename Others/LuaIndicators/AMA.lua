...
if (P>0) then 
	...
	if CandleExist(I,ds) then
		...
		if it.l > 1 then
			sum[Ip] = (sum[Ipp] or 0) + math.abs(GetValue(it[Ip], VT, ds) - GetValue(it[Ipp], VT, ds))
		end
		if it.l < P then
			return nil
		elseif it.l == P then
			tmp.p = GetValue(it[Ip],VT, ds)
		elseif it.l > P then
			local Signal = math.abs(GetValue(it[Ip],VT,ds) - GetValue(it[Ippp],VT,ds))
			local Noise = sum[Ip] - (sum[Ippp] or 0)
			if Noise == 0 then Noise = 1 end
			local SSC = (Signal/Noise * (2/(Fn + 1) - 2/(Sn + 1)) + 2/(Sn + 1))^2
			tmp.p = tmp.pp + SSC*(GetValue(it[Ip],VT,ds) - tmp.pp)
		end
		return tmp.p
	end
end
