...
if (P>0) then
	...
	if CandleExist(I,ds) then
		...
		if it.l > 1 then
			if GetValue(it.p,HIGH,ds) > GetValue(it.pp,HIGH,ds) then
				pDM = math.abs(GetValue(it.p,HIGH,ds)-GetValue(it.pp,HIGH,ds))
			else pDM = 0 end
			if GetValue(it.p,LOW,ds) < GetValue(it.pp,LOW,ds) then
				mDM = math.abs(GetValue(it.pp,LOW,ds)-GetValue(it.p,LOW,ds))
			else mDM = 0 end
...
return function (I, ds)
	...
	if CandleExist(I,ds) then
		if I~=it.p then it={pp=it.p, p=I, l=it.l+1} end
		if it.l == 1 then
			return math.abs(GetValue(it.p,DIFFERENCE, ds))
		else
			return math.max(math.abs(GetValue(it.p,DIFFERENCE, ds)), 
				math.abs(GetValue(it.p,HIGH,ds) - GetValue(it.pp,CLOSE,ds)), 
				math.abs(GetValue(it.pp,CLOSE,ds)-GetValue(it.p,LOW,ds)))
		end
	end
return nil
end
